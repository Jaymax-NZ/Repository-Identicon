...
|---|---|---|
| Return-of-control hook | terminal, ANSI | matrix and colour |
| Konsole tab | icon theme PNG, profile | matrix and colour |
...
raster, the vector, the colour, the matrix, the tricolour and both lattices, and
...
**A change to the matrix rule is not a new colour map.** The matrix does not depend
on the colour map, so a matrix change moves every mark under every map at once.
...
GitHub-style: a 5×5 matrix, mirrored, so every identicon is vertically symmetric
...
```
matrix[row][col] = false for all row, col in 0..4

for index in 0..14:
    painted = hexval(h[index]) mod 2 == 0
    col, row = index div 5, index mod 5
    matrix[row][2 - col] = painted
    matrix[row][2 + col] = painted
```
...
The hue is drawn from the same digest as the matrix, so colour and pattern cannot
drift apart:
...
digests and the matrices in `vectors.json`, so the pattern remains pinned by an
...
byte-indexed left-to-right matrix; both were plausible, neither matched what
...
```
.identicon/settings.json                       the seed, what it draws, and how it is spelled
```

**Only the images are files.** An image is a file because nothing but a file
can be pointed at from a README. Everything else a consumer wants — the
colour, the matrix, the tricolour, the lattices — is a field in
`settings.json`, so the integration is one parse rather than six reads, and a
form this specification does not name is built from the facts rather than
waited for.

`settings.json` is not compared byte-for-byte against generated bytes and is
not among the frozen fixtures.

### What `settings.json` holds

Two sections, and the split between them is the whole design.

`identicon` holds **facts**: `current` is what this repository draws from —
`seed`, `colourMap`, `matrix` as five rows of five booleans, `colour` as
`#rrggbb` — and `history` records what those were before, most recent first,
each entry stamped `at` and carrying only the fields that changed.

`renders` holds **spellings** of those facts: `tricolour` as emoji, and
`blockDrawing` with the matrix on each lattice — `sextant`, `octant`, and
`ascii`, which is `[]` on two spaces.

An implementation MUST derive `renders` from `identicon.current` and nothing
else. A glyph is a rendering of a colour and a shape, so a stored glyph alone
would make the pair it came from unrecoverable; the facts are what is
normative, and `renders` is a convenience that must agree with them.

**What a hand may set**: `identicon.current.seed` and
`identicon.current.colourMap`. `apply` writes everything else. An
implementation MUST cope with a file where only those two are present, and MUST
NOT treat a missing derived field as an error.

`ascii` is two characters per cell because a monospace cell is about twice as
tall as it is wide, and a blank cell is two spaces — so a consumer echoing it
MUST quote the value, or word splitting collapses the runs and drops the
leading pad.
...
`vectors.json` pins the **mapping**: for each seed, the MD5 digest, the matrix,
...
The digest and the matrix do not depend on the colour map. Only `foreground`
...
The division is the useful part. A matrix that changed breaks the vectors; a
...
**Separate image files rather than one.** A combined file would be readable by
every tool that knows the format, which is one tool. A README cannot address a
fragment inside a blob, and `![](.identicon/repository-identicon.svg)` is a
whole integration — usable by a consumer that knows nothing about this
specification.

`renders` holds the rendering from *Text, the fallback* below, for a medium
that will take neither an image nor an escape sequence. Storing it means a
consumer in that position needs no Unicode tables and no palette of its own: it
needs a JSON parser, which it needed for the seed anyway.

**The parts are stored, not the whole.** `renders.tricolour` and each entry of
`renders.blockDrawing` stand alone, so a caller with room for one line and not
two, or for colour and not pattern — a shell prompt, a tab title, a status
field — takes the part it has room for. A caller that wants both composes them,
which is one concatenation rather than a file that has to be split.

**Every lattice is stored.** Which one a host can draw is a fact about its
fonts, and neither the seed nor this specification knows it. An implementation
MUST store all three. Sextants are Unicode 13.0 against the octants' 16.0, so a
consumer choosing without knowing its host SHOULD take the sextant; `ascii`
needs no Unicode at all and is the one that always draws.
...
an approximation of a 5×5 matrix and a colour; an inline image is both, exactly.
...
3. **A lattice plus the tricolour**, below. Two lines, always: the matrix does
...
- **The matrix cannot be one line.** Five rows over either lattice is two text
  ...
  take the matrix at all. Its only option is the tricolour alone, or the badge
  label.
- **The block characters are monochrome, and that is where colour has to come
  from.** The matrix is one glyph per four or six cells, so a cell is not
...
matrix and nothing else — no seed, no digest — so it can be vendored on its own
into a tool with no identicon machinery. The matrix feeds the arrangement as well
...
degraded version of the other**. Both put the whole 5×5 matrix in three
...
eight subcell rows hold a five-row matrix with one or three to spare; all of them
...
emitted.** Where an image can be sent, send the image: it is the matrix, at full
...
- **Colour is never the only channel.** The matrix is the identity; either lattice
...
- **Order is a channel, and it is colour-blind.** Which three appear answers
  *what colour*; the order they appear in is separate information that survives
  any colour deficiency intact. It comes from the matrix, so it is independent of
...
and the tricolour. The lattice remains and is emitted alone: the matrix is the
